...
def kernel_unified_attention_2d(
    ...
    for j in range(tile_start, tile_end):
        ...
        K_load = tl.load(
            ...
            other=0.0,
            eviction_policy="evict_last",  # [终稿 1.4] 保复用、减 HBM
        )
...
def unified_attention(
    ...
    # [终稿 3.1] ROCm 且 hd>=256 长序列:BLOCK_M 提升并规范为 next_power_of_2(max(Bm,32)),
    #            满足 Triton 2 次幂约束并摊销长 KV(Qwen3.5 GQA=24/4=6 -> Bm=32, BQ=5)。
    # [终稿 4.1] 大瓦片 Prefill FA:仅长 Prefill(q>1 且 k>=4096)BLOCK_M>=128。
    use_large_qwen_prefill_tile = (
        current_platform.is_rocm() and head_size == 256 and num_queries_per_kv == 6
        and max_seqlen_q > 1 and max_seqlen_k >= 4096  # 仅长 Prefill
    )
    if current_platform.is_rocm() and head_size >= 256 and max_seqlen_q > 1             and num_queries_per_kv > 1:
        BLOCK_M = triton.next_power_of_2(max(BLOCK_M, 32))
    if use_large_qwen_prefill_tile:
        BLOCK_M = triton.next_power_of_2(max(BLOCK_M, 128))
    ...
    # [终稿 5.1] 大瓦片 Prefill FA:TILE_N 32->64(环境可配),禁深流水控 LDS
    num_warps, num_stages = None, None
    if use_large_qwen_prefill_tile:
    ...
    if (
        ...
        kernel_unified_attention_2d[
            ...
            **(dict(num_warps=num_warps, num_stages=num_stages)
               if num_warps is not None else {}),
        )
    else:
