...
def get_block_size(dtype: torch.dtype, head_size: int | None = None) -> int:
    if dtype == torch.float32:
        return 32
    if current_platform.is_rocm() and head_size is not None and head_size >= 256:
        return 32  # [终稿 1.2] DCU 且 hd>=256:BLOCK=32,控 shared 压 TTFT
...
def get_prefill_tiles(dtype, head_size, max_input_len):
    """[终稿 2.2] 非对称 Prefill 瓦片 + 软件流水:返回 (Bm, Bn, warps, stages)。"""
...
def context_attention_fwd(
    ...
    # [终稿 2.2] ROCm 长输入走非对称瓦片;否则沿用 BLOCK 对称瓦片
    tiles = None
    if current_platform.is_rocm():
        tiles = get_prefill_tiles(q.dtype, Lq, max_input_len)
    if tiles is not None:
        BLOCK_M, BLOCK_N, num_warps_fwd, num_stages_fwd = tiles
    else:
        BLOCK_M, BLOCK_N, num_warps_fwd, num_stages_fwd = BLOCK, BLOCK, (4 if Lk <= 64 else 8), 1
