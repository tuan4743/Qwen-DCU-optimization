...
# [DCU Optimize] MUST run before `import torch` (eval may omit start_vllm.sh).
# Goal: Decode TPOT→~45–47ms / Peak≥21 on gfx936 BW1000 single-card.
# Bandwidth: M=1 rocBLAS ≫ hipBLASLt (HIPBLASLT=1 → TPOT~60ms / Peak~17).
# Force=0 so stale shell exports of 1 cannot override. Prefill A/B escape:
#   VLLM_ROCM_FORCE_HIPBLASLT=1
# Precision: env only; no weight/KV dtype change. KV FP8 stays default-off.
if os.getenv("VLLM_ROCM_FORCE_HIPBLASLT", "0").lower() in ("1", "true", "yes"):
...
os.environ.setdefault("VLLM_ROCM_KV_FP8", "0")
os.environ.setdefault("FLA_GDN_FIX_BT", "1")
os.environ.setdefault("VLLM_ROCM_USE_SKINNY_GEMM", "1")
os.environ.setdefault("VLLM_ROCM_USE_AITER", "0")
# Prefill GDN: fused cumsum+KKT+tril+WU — DEFAULT OFF after A/B
# (2026-07-14): TTFT flat/worse on 16–32K; set =1 only for re-bench.
os.environ.setdefault("VLLM_ROCM_GDN_FUSED_PREPROC", "0")
# FA Prefill KV tile for Qwen GQA6 long context (32=0425 已知好点; 64=本轮试探)
os.environ.setdefault("VLLM_ROCM_FA_PREFILL_TILE", "64")
