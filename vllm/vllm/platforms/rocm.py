...
# [DCU Optimize] Soft-import _rocm_C. Catch all exceptions: a broken/mismatched
# .so can raise more than ImportError and would otherwise kill EngineCore.
try:
...
# Stock GFX9 = MI200/MI300 family only.
# [DCU Optimize] Keep gfx936 OUT of _ON_GFX9 so AITER/FA/custom-attn gates
# stay MI-only. Skinny GEMM for DCU is enabled via on_gfx936() + ops probe
# in model_executor/layers/utils.py (not by widening _ON_GFX9).
_ON_GFX9 = any(arch in _GCN_ARCH for arch in ["gfx90a", "gfx942", "gfx950"])
_ON_GFX942 = "gfx942" in _GCN_ARCH
_ON_GFX950 = "gfx950" in _GCN_ARCH
_ON_GFX936 = any(arch in _GCN_ARCH for arch in ["gfx936", "gfx928", "gfx926", "gfx938"])
...
@cache
def rocm_skinny_ops_available() -> bool:
    """
    [DCU Optimize] True only when _rocm_C skinny GEMM ops are registered.

    Goal: allow gfx936 decode Linear to use wvSplitK/LLMM1 without crashing
    eval machines that never built _rocm_C (ops missing → fall back F.linear).
    Context: Qwen3.5-27B decode M=1 GEMV (8k~16k score bucket).
    Precision: N/A (availability probe only).
    """
    try:
        import torch
...
class RocmPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_config: "VllmConfig") -> None:
        ...
        # [DCU Optimize] Optional runtime KV FP8 on gfx936 (env-gated).
        # Goal: cut KV HBM traffic on 8k~16k decode; Precision: validate Δ<1%
        # before enabling in scoring runs. No offline weight quant.
        if (
            cache_config
            and getattr(cache_config, "cache_dtype", "auto") == "auto"
            and envs.VLLM_ROCM_KV_FP8
            and on_gfx936()
            and cls.supports_fp8()
        ):
            from vllm.cache import recommend_kv_cache_dtype

            kv_dtype = recommend_kv_cache_dtype(prefer_fp8=True, supports_fp8=True)
            if kv_dtype is not None:
                cache_config.cache_dtype = kv_dtype
                logger.warning_once(
                    "[DCU] VLLM_ROCM_KV_FP8=1 → cache_dtype=%s (runtime KV only).",
                    kv_dtype,
                )
    ...
    def supports_fp8(cls) -> bool:
        # [DCU Optimize] gfx936 reports as gfx93x; allow runtime KV FP8 path.
        return any(
            gfx in _GCN_ARCH for gfx in ["gfx94", "gfx95", "gfx93", "gfx92", "gfx12"]
        )

    @classmethod
    def is_fp8_fnuz(cls) -> bool:
        # [DCU Optimize] Hygon DCU gfx93x uses FP8 FNUZ (same family as MI300
        # gfx94). Needed so runtime KV FP8 scales/casts match e4m3fnuz cache.
        return any(gfx in _GCN_ARCH for gfx in ["gfx94", "gfx93", "gfx92"])
