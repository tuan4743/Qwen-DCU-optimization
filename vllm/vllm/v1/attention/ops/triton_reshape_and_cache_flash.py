...
def triton_reshape_and_cache_flash(
    ...
    # [DCU Optimize] Prefill KV write (TTFT): ROCm stages≤3 (stages=4 risks
    # LDS/reg pressure on hd≥256). Cap TILE@1024 when n≥256. Precision: copy.
    TILE_SIZE = min(2048, triton.next_power_of_2(n))
    if current_platform.is_rocm() or current_platform.is_xpu():
        num_stages = 3
        num_warps = 8
        if n >= 256:
            TILE_SIZE = min(1024, TILE_SIZE)
    else:  # cuda
        num_stages = 10
        num_warps = 16
...
def triton_reshape_and_cache_flash_diffkv(
    ...
    # heuristics instead of autotuning
    # [DCU Optimize] ROCm KV write: stages=3 (not 4) under LDS budget.
