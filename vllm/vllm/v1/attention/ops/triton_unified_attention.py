...
def kernel_unified_attention_2d(
    ...
    for j in range(tile_start, tile_end):
        ...
        # [DCU Optimize] evict_last only (no TILE/stages change): keep streaming
        # KV in cache on long decode; matches chunked_prefill_paged_decode.
        # Goal: lower TPOT via better HBM reuse; Precision: unchanged.
        K_load = tl.load(
...
def _get_tile_size(
    ...
    # Default behavior
    # [DCU Optimize] Qwen3.5 hd=256 long prefill: TILE=16 + deeper stages
    # (set at launch) streams KV under 64KB LDS; TILE=32 + stages≥3 OOMs.
    if is_prefill:
        if current_platform.is_rocm() and head_size >= 256:
            return 16
        return 32
    return 16 if element_size >= 2 else 32


def _rocm_unified_launch_cfg(
    max_seqlen_q: int,
    max_seqlen_k: int,
    head_size: int,
) -> tuple[int, int]:
    """
    [DCU Optimize] num_warps / num_stages for unified attention on gfx936.

    Prefill (q>1): TTFT @ 4k–32k; stages=3 + warps=8 hide HBM on TILE=16
    long-KV loops. Threshold 4K (not 8K) covers mid-chunked extend.
    Decode (q==1): longwarps4 — warps=4, stages=2 (K≥16K warps=8 regressed
    occupancy on B=1). Never stages=4 / Decode TILE=32 (负优化黑名单).
    Precision: launch config only, softmax math unchanged.
    """
    if not (current_platform.is_rocm() and head_size >= 256):
        return 4, 1
    if max_seqlen_q > 1:
        # Chunked FA (q < k) dominates 8–32K TTFT after first dense chunk.
        if max_seqlen_k >= 4096:
            return 8, 3
        return 8, 2
    # Decode Flash-Decoding / 2D — longwarps4 (XD c029: +1.84% / −2.76% TPOT)
    return 4, 2
...
def unified_attention(
    ...
    # [DCU Optimize] Qwen3.5 FA Prefill large tile (0425: M=128+N=32 → TTFT 腰斩).
    # Next knife (0700): N=32→64 halves long-KV loop count again; keep stages=1
    # under 64KB LDS (stages≥2 with large N historically OOM / 负优化).
    # Override: VLLM_ROCM_FA_PREFILL_TILE=32 回退已知好点. Decode 仍 TILE=16.
    # Precision: same softmax math; tile geometry only.
    ...
    if (
        ...
        min_block_m = 128 if use_large_qwen_prefill_tile else 32
        BLOCK_M = triton.next_power_of_2(max(BLOCK_M, min_block_m))
    ...
    if use_large_qwen_prefill_tile:
        ...
        try:
            tile_n = int(os.getenv("VLLM_ROCM_FA_PREFILL_TILE", "64"))
        except ValueError:
            tile_n = 64
        # Power-of-two tile only; clamp to safe set.
        if tile_n not in (16, 32, 64):
            tile_n = 64
        TILE_SIZE_PREFILL = tile_n
    ...
    num_warps, num_stages = _rocm_unified_launch_cfg(
        max_seqlen_q, max_seqlen_k, head_size
    )
    if use_large_qwen_prefill_tile:
        # stages=1: large M×N working set must stay in gfx936 LDS/VGPR budget.
    ...
    if (
        ...
        # Decode keeps TILE=16. Long Qwen Prefill uses TILE_N above.
        tile_2d = (
            TILE_SIZE_DECODE
