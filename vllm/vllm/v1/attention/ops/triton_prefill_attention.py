...
def get_prefill_tiles(
    dtype: torch.dtype, head_size: int, max_input_len: int
) -> tuple[int, int, int, int]:
    """
    Return (BLOCK_M, BLOCK_N, num_warps, num_stages) for Triton prefill.

    [DCU Optimize] gfx936 capability 9.x falsely triggers BLOCK=128 path.
    hd=256 LDS≈2*(Bm+Bn)*256*2B; keep ≤64KB.
    8k~16k score bucket: use Bm=32,Bn=16 + stages=3 to hide HBM on long
    KV scan (TTFT dominates wall-clock output tok/s when TPOT already ~47ms).
    Precision: tile/pipeline only, softmax math unchanged.
    """
    if dtype == torch.float32:
        return 32, 32, 4, 1

    if current_platform.is_rocm() and head_size >= 256:
        # Dense Prefill (q==k, chunk=4096) hit LDS OOM with Bm=Bn=32 +
        # stages=3 (Required 67584 > 65536). Use asymmetric Bn=16 under
        # 64KB cap — same geometry for mid/long. Precision: tile only.
    ...
    if current_platform.is_cuda_alike() and current_platform.has_device_capability(80):
        ...
        b = 64
    return b, b, (4 if head_size <= 64 else 8), 1
...
def context_attention_fwd(
    ...
    BLOCK_M, BLOCK_N, num_warps, num_stages = get_prefill_tiles(
        q.dtype, Lk, max_input_len
    )
