...
# [DCU Optimize] Flash-Decoding segments for B≈1 long decode.
# 64 caused 16–32K wall-clock regression (extra reduce/launch tax).
# Cap 32; runtime may use fewer via _flash_decode_segments().
NUM_PAR_SOFTMAX_SEGMENTS = 32


def _flash_decode_segments(max_seq_len: int, max_segments: int) -> int:
    """
    [DCU Optimize] Adaptive Flash-Decoding segment count.

    Goal: 8k~16k / 16k~32k decode CU fill without over-segmentation.
    Context: TILE_SIZE_DECODE≈16; ~256 tok/segment → segs≈ceil(S/256).
    Bandwidth: more segments hide HBM on long KV; too many → reduce tax.
    Precision: online softmax reduce in fp32 — math unchanged.
    """
    if max_seq_len <= 2048:
        return min(16, max_segments)
    ideal = (max_seq_len + 255) // 256
    segs = 1 << max(ideal - 1, 0).bit_length()
...
class TritonAttentionMetadataBuilder(AttentionMetadataBuilder[TritonAttentionMetadata]):
    ...
    def __init__(
        ...
        # Buffer sized for the max; per-step may pass a smaller segment count.
    ...
    def build(
        ...
        # Flash-Decoding segments: keep fixed when CUDA Graphs capture decode
        # (grid Z = num_segments must match the captured graph). Adaptive
        # only in eager mode.
        if (
            max_query_len <= 1
            ...
            num_par_segs = _flash_decode_segments(
                max_seq_len, self.num_par_softmax_segments
            )
        else:
            num_par_segs = self.num_par_softmax_segments
        ...
        attn_metadata = TritonAttentionMetadata(
            ...
            num_par_softmax_segments=num_par_segs,
...
class TritonAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        # [DCU Optimize] Dense FA (q==k→context_attention_fwd) removed for
        # 16–32K push: 0311 实测 Mean TTFT 12.2s 纹丝不动，且曾 LDS OOM。
        # 长档 Output 主战场改打 Prefill 权重 GEMM 双路径（utils.py）。
        # Keep unified_attention for all decoder FA Prefill/Decode.
