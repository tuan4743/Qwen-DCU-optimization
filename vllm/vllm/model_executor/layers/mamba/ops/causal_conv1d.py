...
def causal_conv1d_fn(
    ...
    num_cache_lines = 0
    # [DCU Optimize] gfx936 Prefill TTFT: BLOCK_M=8 → 512 programs / 4096-tok
    # chunk × 48 GDN layers. BLOCK_M=16 halves launches; metadata covers {8,16}.
    # Bandwidth: fewer grid launches + better token locality. Precision: same math.
    try:
        from vllm.platforms import current_platform as _plat

        BLOCK_M = 16 if _plat.is_rocm() else 8
    except Exception:
        BLOCK_M = 8
    ...
    _causal_conv1d_fwd_kernel[grid](
        ...
        num_stages=3 if BLOCK_M >= 16 else 2,
    )
...
def causal_conv1d_update(
    ...
    _causal_conv1d_update_kernel[grid](
        ...
        USE_PAD_SLOT=pad_slot_id is not None,
        BLOCK_N=256,
        # [DCU Optimize] Decode GDN conv update (TPOT / Output): warps=4,
        # stages=2 on ROCm — fewer tiny waves. Precision: same conv+silu.
        num_warps=4,
        num_stages=2,
    )
    if unsqueeze:
