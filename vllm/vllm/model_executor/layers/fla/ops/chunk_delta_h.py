...
# [DCU Optimize] Narrow ROCm autotune for GDN state recurrence (Prefill TTFT).
# Qwen3.5: K=V=128, BT=64. warps=4 only + stages∈{2,3} (not single-pin).
...
@triton.autotune(
    configs=[
        ...
        for num_warps in _H_WARPS
        for num_stages in _H_STAGES
        for BV in _H_BV
    ],
    key=["H", "K", "V", "BT"],
