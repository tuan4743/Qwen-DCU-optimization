...
from .fused_chunk_preprocessing import fused_chunk_preprocessing_fwd
...
def _use_fused_gdn_preprocessing() -> bool:
    """[DCU Optimize] Prefill GDN fused preprocessing gate.

    Default OFF after 2026-07-14 A/B: TTFT flat/slightly worse on long tiers
    vs unfused (16–32K 6.67→6.73s). Set VLLM_ROCM_GDN_FUSED_PREPROC=1 to retry.
    """
    env = os.getenv("VLLM_ROCM_GDN_FUSED_PREPROC")
    if env is not None:
...
def chunk_gated_delta_rule_fwd(
    ...
    if _use_fused_gdn_preprocessing():
        g, w, u = fused_chunk_preprocessing_fwd(
