...
#
# Fused GDN chunk preprocessing (vLLM #38787), adapted for gfx936 ROCm.
# ruff: noqa: E501
...
# [DCU Optimize] Prefill GDN: fuse cumsum+KKT+tril+WU (vLLM #38787).
# Goal: cut TTFT by removing A[B,T,H,BT] HBM + 3 kernel launches ×48 GDN.
# ROCm: narrow autotune (not v8 single-pin). Precision: allow_tf32=False;
# keep FLA_TRIL ieee path available via env fallback to unfused.
...
@triton.autotune(
    configs=[
        ...
        for BK in _PRE_BK
        for BV in _PRE_BV
        for num_warps in _PRE_WARPS
        for num_stages in _PRE_STAGES
    ],
