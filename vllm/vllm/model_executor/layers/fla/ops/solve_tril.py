...
# [DCU Optimize] Prefill GDN solve_tril: ROCm warps=4 × stages∈{2,3} (not
# stages=4/5 which inflate LDS). Avoids v8-style single-config pin.
_TRIL_WARPS_16 = [4] if is_amd else [1, 2, 4, 8]
_TRIL_STAGES_16 = [2, 3] if is_amd else [2, 3, 4, 5]
_TRIL_WARPS_32 = [4] if is_amd else [1, 2, 4, 8]
_TRIL_STAGES_32 = [2, 3] if is_amd else [2, 3, 4, 5]
_TRIL_WARPS_64 = [4] if is_amd else [2, 4, 8]
_TRIL_STAGES_64 = [2, 3] if is_amd else [2, 3, 4, 5]
...
@triton.autotune(
    configs=[
        ...
        for num_warps in _TRIL_WARPS_16
        for num_stages in _TRIL_STAGES_16
    ],
    key=["BT"],
)
...
@triton.autotune(
    configs=[
        triton.Config({}, num_warps=num_warps, num_stages=num_stages)
        for num_warps in _TRIL_WARPS_32
        for num_stages in _TRIL_STAGES_32
    ],
    key=["H", "BT", "IS_VARLEN"],
)
@triton.jit(do_not_specialize=["T"])
...
@triton.autotune(
    configs=[
        triton.Config({}, num_warps=num_warps, num_stages=num_stages)
        for num_warps in _TRIL_WARPS_64
        for num_stages in _TRIL_STAGES_64
    ],
    key=["H", "BT", "IS_VARLEN"],
)
@triton.jit(do_not_specialize=["T"])
