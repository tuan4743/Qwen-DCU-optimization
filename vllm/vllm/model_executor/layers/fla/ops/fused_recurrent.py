...
def fused_recurrent_gated_delta_rule_packed_decode(
    ...
    # [DCU Optimize] Qwen3.5-27B decode GDN: HV=48,V=K=128. BV=32 → 4 programs
    # per value-head reloading q/k/gating. BV=128 + warps=4 cuts duplicate HBM
    # and launch tax → lower TPOT / higher Output (stage A: 21/16/15). Other
    # shapes keep conservative BV≤32. Precision: same recurrence math.
    is_qwen35_decode = HV == 48 and V == 128 and K == 128
    BV = 128 if is_qwen35_decode else min(triton.next_power_of_2(V), 32)
    num_stages = 3
    num_warps = 4 if is_qwen35_decode else 1
