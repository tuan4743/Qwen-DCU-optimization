...
# [DCU Optimize] Prefill GDN q/k L2: fewer warps + larger MBLOCK on ROCm.
_ROCM = False
try:
    _ROCM = bool(current_platform.is_rocm())
except Exception:
    _ROCM = False
_L2_WARPS = [2, 4, 8] if _ROCM else [1, 2, 4, 8, 16, 32]
_L2_BT_WARPS = [2, 4, 8] if _ROCM else [1, 2, 4, 8, 16]
_L2_BT_LIST = [32, 64, 128] if _ROCM else BT_LIST


@triton.autotune(
    configs=[triton.Config({}, num_warps=num_warps) for num_warps in _L2_WARPS],
    key=["D"],
)
@triton.jit
...
@triton.autotune(
    configs=[
        ...
        for num_warps in _L2_BT_WARPS
        for BT in _L2_BT_LIST
    ],
    key=["D"],
)
...
def l2norm_fwd(
    ...
    if not USE_DEFAULT_FLA_NORM:
        # [DCU Optimize] MBLOCK 32→64 on ROCm: half programs / Prefill GDN L2.
        MBLOCK = 64 if _ROCM else 32
        # M, N = x.shape
