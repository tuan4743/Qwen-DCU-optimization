...
class Qwen3NextGatedDeltaNet(nn.Module, MambaBase):
    ...
    def __init__(
        ...
        # [DCU Optimize] Fused Decode in_proj (from lsd): one GEMV over
        # concatenated [qkvz; ba] vs two launches ×48 GDN/step. Prefill
        # unchanged (num_tokens>1). Precision: same matmul, view split only.
        self._fused_in_proj_weight: torch.Tensor | None = None
        self._qkvz_out_size: int = 0

    def fuse_in_proj_for_decode(self) -> bool:
        """Merge in_proj_qkvz + in_proj_ba into one contiguous weight buffer.

        Re-points both sub-module weights at contiguous views of the merged
        buffer so single-token decode can issue one GEMV. The caller releases
        allocator-cached original storage after all layers have been fused.
        """
        ...
        if self.quant_config is not None:
            return False
        try:
            from vllm.platforms.rocm import on_gfx936, rocm_skinny_ops_available
        except Exception:
            return False
        if not (
            ...
            return False
        wq = self.in_proj_qkvz.weight.data
        wb = self.in_proj_ba.weight.data
        if (
            wq.dim() != 2
            or wb.dim() != 2
            or wq.shape[1] != wb.shape[1]
            or wq.device != wb.device
            or wq.dtype != wb.dtype
            or wq.dtype not in (torch.float16, torch.bfloat16)
            or wq.shape[1] % 8 != 0
        ):
            return False
        merged = torch.cat([wq, wb], dim=0)
        split = wq.shape[0]
        self.in_proj_qkvz.weight.data = merged[:split]
        self.in_proj_ba.weight.data = merged[split:]
        self._fused_in_proj_weight = merged
        self._qkvz_out_size = split
        return True
    ...
    def forward(
        ...
        # ============================================================
        if self._fused_in_proj_weight is not None:
            # [DCU Optimize] Fused qkvz∥ba for Prefill+Decode: one GEMM/GEMV.
            # Prefill: skip .contiguous() on views (avoid 2× large copies).
            fused_proj = torch.ops.vllm.rocm_unquantized_gemm(
                hidden_states, self._fused_in_proj_weight, None
            )
            qkvz = fused_proj[..., : self._qkvz_out_size]
            ba = fused_proj[..., self._qkvz_out_size :]
            if num_tokens == 1:
                projected_states_qkvz = qkvz.contiguous()
                projected_states_ba = ba.contiguous()
            else:
...
def fuse_gdn_input_projections_for_decode(module: nn.Module) -> int:
    """Fuse eligible GDN projections and release replaced weight storage."""
    num_fused = 0
    for submodule in module.modules():
        if isinstance(submodule, Qwen3NextGatedDeltaNet):
            num_fused += int(submodule.fuse_in_proj_for_decode())

    if num_fused:
        # ROCm model-memory profiling counts allocator-reserved blocks.
        torch.accelerator.empty_cache()
    return num_fused
...
class Qwen3NextForCausalLM(
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        fuse_gdn_input_projections_for_decode(self)
        return loaded
