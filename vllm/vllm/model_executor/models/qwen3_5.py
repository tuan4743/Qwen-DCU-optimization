...
from .qwen3_next import (
    ...
    fuse_gdn_input_projections_for_decode,
)
...
class Qwen3_5GatedDeltaNet(Qwen3NextGatedDeltaNet):
    ...
    def forward(
        ...
        if self._fused_in_proj_weight is not None:
            # Prefill+Decode fused in_proj (same buffer as load_weights fuse).
            fused_proj = torch.ops.vllm.rocm_unquantized_gemm(
                hidden_states, self._fused_in_proj_weight, None
            )
            mixed_qkvz = fused_proj[..., : self._qkvz_out_size]
        ...
        # Decode packed kernel overwrites every element — skip zero fill ×48.
        alloc_core_out = torch.empty if num_tokens == 1 else torch.zeros
        core_attn_out = alloc_core_out(
