...
// [DCU Optimize] Enable skinny GEMV/GEMM (LLMM1/wvSplitK) on gfx936 DCU.
// Goal: Decode M=1 Linear memory-bound path; 128-bit loads + LDS reuse.
// Context: Qwen3.5-27B decode TPOT; Precision: same bf16/fp16 math as MI path.
#if defined(__HIPCC__) &&                                                 \
    (defined(__gfx90a__) || defined(__gfx942__) || defined(__gfx950__) || \
     defined(__gfx936__) || defined(__gfx928__) || defined(__gfx926__) || \
     defined(__gfx938__))
  #define __HIP__GFX9__
  // cuda_compat.h WARP_SIZE uses __GFX9__ (wave64). Hygon may not set it;
  // force wave64 so LLMM1 K-reduction does not drop half the partials.
  #ifndef __GFX9__
    #define __GFX9__ 1
  #endif
#endif

// [DCU Optimize] Hygon DCU family: DPP wave-reduce is unreliable → use shfl.
#if defined(__gfx936__) || defined(__gfx928__) || defined(__gfx926__) || \
    defined(__gfx938__)
  #define __HIP__DCU_GFX93X__ 1
#endif
...
#pragma unroll
  for (int i = 0; i < NUM_A_ROWS_PER_BLOCK; i++) {
    // [DCU Optimize] fp32 accumulate (match F.linear / hipBLAS), not bf16
    // __hfma2 chain which drifts ~2% on long-K decode GEMV (K up to 8192).
    // Bandwidth: same 128-bit loads; Precision: fp32 acc of pairwise products.
    float local_acc = 0.f;
    if (threadid * 8 < K) {
      ah2lptr = Ah2ptr + i * 4;
      const scalar2_t bvec[4] = {colB_elem4x, colB_elem4y, colB_elem4z,
                                 colB_elem4w};
#pragma unroll
      for (int p = 0; p < 4; p++) {
        float2 av = __s22float2(*(ah2lptr + p));
        float2 bv = __s22float2(bvec[p]);
        local_acc += av.x * bv.x + av.y * bv.y;
      }
    }
    acc[i] = local_acc;
  }
...
// [DCU Optimize] DOT2C for skinny GEMV on gfx9.
// - MI (gfx90a/942/950): use v_dot2c_f32_f16 ISA
// - gfx936/DCU: that instruction is unsupported → portable fp16→fp32 mul-add
// Goal: compile+run wvSplitK on competition DCU; Precision: fp32 acc of 2 products.
  ...
  #define DOT2C(V0, V2, V3)                                                   \
    if constexpr (std::is_same_v<scalar_t, half>) {                           \
      float2 s = __half22float2(*((const half2*)(&(V2)))) *                   \
                 __half22float2(*((const half2*)(&(V3))));                    \
      V0 += (s.x + s.y);                                                      \
    } else if constexpr (std::is_same_v<scalar_t, __hip_bfloat16>) {          \
      float2 s = __bfloat1622float2(*((__hip_bfloat162*)(&(V2)))) *           \
                 __bfloat1622float2(*((__hip_bfloat162*)(&(V3))));            \
...
// [DCU Optimize] Wave64 sum-reduce via shfl_xor (portable on gfx936).
// Goal: replace broken mov_dpp epilogue so wvSplitK matches F.linear.
// Context: Decode M>8 skinny path; Bandwidth: N/A (epilogue only).
// Precision: fp32 lane fold; all lanes get full sum after tree.
__device__ __forceinline__ float wv_wave64_reduce_sum(float v) {
#pragma unroll
...
__global__ void __launch_bounds__(WvPrGrp* THRDS)
  ...
  #pragma unroll
    ...
    if constexpr (!use_mfma) {
      for (int n = 0; n < N; n++) {
        for (int y = 0; y < YTILE; y++) {
#if defined(__HIP__DCU_GFX93X__)
          // gfx936: mov_dpp wave-fold is unreliable → shfl_xor tree (all lanes).
          sum[n][y] = wv_wave64_reduce_sum(sum[n][y]);
#else
