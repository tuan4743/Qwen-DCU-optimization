# 03 · 仓库导览:目录与用途

> 一句话:本仓库是"最终成果 + 全程记录"的合集——想要结论看根目录 README,想要过程看 `optimization_records/`,想要代码对照看 `vllm_final/` 与 `vllm_origin/`。

## 目录总览

| 目录/文件 | 里面是什么 | 给谁看 |
|---|---|---|
| `README.md` | 门面:成绩、硬件与适配坑、五轮汇总 | 所有人(先看这个) |
| `final_optimization_report.md` | 完整故事:背景、基线、五轮细节、经验与风险 | 想深入了解的人 |
| `vllm_final/` | **最终修改代码**(12 个文件,相对 vllm_origin 的差异就是全部改动) | 复现/集成的人 |
| `vllm_origin/` | 修改前基线(同 12 文件,来自 fork 基线) | 对照用 |
| `vllm_final/changelog_list.md` | 每个文件改了什么、依据、注意事项 | 复现的人 |
| `optimization_records/` | 过程全记录:01-19 任务文档 + 终稿说明文档 + 40 张截图 + 工具脚本 + 日志 | 想复盘的人(按时间顺序,长) |
| `profile/` | 性能画像物证:批3 trace(gz)、pmc 结果、hipkernel 结果、kernel 参数表 | 怀疑性能数字的人 |
| `docs/` | 本目录:成果总览 / 改动详解 / 融合说明 / 导览 / 底稿 | 所有人 |

## 几个高频问题

**"我想在别的国产卡上复现,从哪开始?"**
`docs/01_final_changes_spec.md`(15 处改动的完整对照)→ 按 1.1 → 其余轮次顺序推进;`README.md`「Hardware Setup」里是必须先解决的适配坑清单。

**"最终代码和我手上的 vLLM 怎么合?"**
差异就 12 个文件:`vllm_final/` 与 `vllm_origin/` 同名对照即可;其中 `csrc/rocm/skinny_gemms.cu` 需要跟着重新编译,其余是 Python 文件(替换即可)。

**"这些成绩是真的吗?"**
每条数字都有出处:五轮成绩表见说明文档汇总表,原始 trace/PMC 数据在 `profile/`,截图在 `optimization_records/attachments/`,两边的数字互相咬合可查。

**"早期文档说某方向不对,又看到最终成果里用了它?"**
以 `docs/02` 的说明为准:早期记录截至 07-13,之后发生的五轮优化未纳入;"融合"在早期文档里指的对象(跨算子单 kernel)与最终落地(权重拼接/瓦片/融核)不同。

## 原始素材位置(本仓库之外)

- 原始工作区 `vllm_optimize_data/`(源码树 vllm_cscc、tasks 01-19、tools、log)保留未动,本仓库的 `optimization_records/` 是其中与成果相关的精选副本。
