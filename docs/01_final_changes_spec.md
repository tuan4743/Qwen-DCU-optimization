# 01 · 改动详解(15 个子项,给动手复现的人)

> 用途:把 `vllm_final/` 的改动逐项讲清楚——每个子项给出:改哪个文件、改前什么样、改后怎么样、为什么这么改,并附对应截图。
> 代码来自终稿说明文档与截图(40 张);`[?]`=个别字符存疑,以原始截图为准。
> 复现顺序建议:先 1.1b(HIP 编译宏,否则算子是空壳)再 1.1a(路由),之后按轮次 1→5 逐轮验证。
> 行号锚点基于 `vllm_origin/` 基线;最终代码以 `vllm_final/` 为准(本表是"为什么/怎么用"的说明书)。

> **给读者的话**:不想动手就不用看这份,成果在 `00_audit_local_vs_final.md` 与根目录 README。这份是给想落地/移植的人的详表。
