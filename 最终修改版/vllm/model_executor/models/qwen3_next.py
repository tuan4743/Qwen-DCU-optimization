...
class Qwen3NextGatedDeltaNet(nn.Module, MambaBase):
    ...
    def __init__(
        ...
        # [终稿 5.2] 融合权重:首次 forward 时由 qkvz/ba 两套权输出维拼接(不落盘、不改数值),
        # 数学上 Y = X[Wqkvz; Wba] 与分乘再拼等价。
    ...
    def forward(
        ...
        # ============================================================
        # [终稿 4.3+5.2] in_proj 融合:单次 GEMV/GEMM;decode(num_tokens==1)保 contiguous,
        # Prefill 直接用 view,避免 T×out 二次 HBM 写。
        from vllm.model_executor.layers.utils import rocm_unquantized_gemm
        if self._fused_in_proj_weight is None:
            self._fused_in_proj_weight = torch.cat(
                [self.in_proj_qkvz.weight, self.in_proj_ba.weight], dim=0
            )
        fused = rocm_unquantized_gemm(hidden_states, self._fused_in_proj_weight, None)
        qkvz_shape = self.in_proj_qkvz.weight.shape[0]
        projected_states_qkvz, projected_states_ba = (
            fused[..., :qkvz_shape], fused[..., qkvz_shape:]
        )
        if num_tokens == 1:
            projected_states_qkvz, projected_states_ba = (
