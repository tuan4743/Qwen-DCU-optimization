...
NUM_PAR_SOFTMAX_SEGMENTS = 32  # [终稿 1.3+2.1] 长上下文并行上限 16->32(提升长上下文 Decode 并行度)


@dataclass
def _flash_decode_segments(max_seq_len, max_segments):
    """[终稿 2.1] eager 按 seq 自适应分段数(2 次幂,约 256 tok/段),上限 max_segments。"""
    if max_seq_len <= 2048:
...
class TritonAttentionMetadataBuilder(AttentionMetadataBuilder[TritonAttentionMetadata]):
    ...
    def build(
        ...
        # [终稿 2.1] cudagraph 捕获时固定段数(捕获期 grid Z 与自适段数冲突);
        # eager 按 seq 自适应;上限 NUM_PAR_SOFTMAX_SEGMENTS(32)。
        if not self.decode_cudagraph_enabled:
            self.num_par_softmax_segments = _flash_decode_segments(
                max_seq_len, NUM_PAR_SOFTMAX_SEGMENTS
            )
